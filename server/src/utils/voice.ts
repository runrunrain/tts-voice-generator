...
export interface GeminiVoiceCatalogEntry {
   ...
   * All prebuilt Gemini TTS voices are classified as either male or female.
   */
  perceivedGender: VoicePerceivedGender;
}

export const GEMINI_VOICE_CATALOG: readonly GeminiVoiceCatalogEntry[] = [
  { name: "Zephyr", displayNameZh: "和风", toneZh: "明亮、自然", selectionHints: ["明亮旁白", "自然叙述", "默认自然声"], perceivedGender: "female" },
  ...
  { name: "Callirrhoe", displayNameZh: "卡利罗厄", toneZh: "放松、舒缓", selectionHints: ["放松叙述", "舒缓说明", "慢节奏"], perceivedGender: "female" },
  { name: "Autonoe", displayNameZh: "奥托诺厄", toneZh: "明快、清亮", selectionHints: ["明快播报", "积极提示", "清晰短句"], perceivedGender: "female" },
  { name: "Enceladus", displayNameZh: "恩克拉多斯", toneZh: "气声、虚弱感", selectionHints: ["耳语", "疲惫", "神秘", "低声"], perceivedGender: "male" },
  { name: "Iapetus", displayNameZh: "伊阿珀托斯", toneZh: "清晰、理性", selectionHints: ["清晰解释", "理性说明", "教程"], perceivedGender: "male" },
  { name: "Umbriel", displayNameZh: "翁布里尔", toneZh: "轻松愉快", selectionHints: ["轻松愉快", "随和", "明亮互动"], perceivedGender: "male" },
  { name: "Algieba", displayNameZh: "阿尔吉巴", toneZh: "平滑、圆润", selectionHints: ["圆润播报", "平顺叙述", "广告旁白"], perceivedGender: "male" },
  ...
  { name: "Schedar", displayNameZh: "王良四", toneZh: "平稳、克制", selectionHints: ["克制叙述", "平稳旁白", "成熟旁白"], perceivedGender: "male" },
  { name: "Gacrux", displayNameZh: "十字架一", toneZh: "成熟、稳健", selectionHints: ["成熟角色", "稳健旁白", "中年叙述"], perceivedGender: "female" },
  { name: "Pulcherrima", displayNameZh: "普尔凯里玛", toneZh: "前瞻、明亮", selectionHints: ["科技感", "未来感", "宣传介绍"], perceivedGender: "female" },
  { name: "Achird", displayNameZh: "阿基尔德", toneZh: "友好、亲切", selectionHints: ["友好对话", "亲切提示", "陪伴感"], perceivedGender: "male" },
  { name: "Zubenelgenubi", displayNameZh: "氐宿一", toneZh: "随意、口语", selectionHints: ["随意闲聊", "市井对白", "自然口语"], perceivedGender: "male" },
  { name: "Vindemiatrix", displayNameZh: "太微左垣四", toneZh: "温和、细腻", selectionHints: ["温和解释", "细腻情绪", "安静对白"], perceivedGender: "female" },
  { name: "Sadachbia", displayNameZh: "萨达克比亚", toneZh: "活泼、灵动", selectionHints: ["活泼", "灵动", "轻快互动", "年轻角色"], perceivedGender: "male" },
...
export function formatVoiceSelectionGuideForPrompt(): string {
  return GEMINI_VOICE_CATALOG
    .map((voice) => `${voice.name}（${voice.displayNameZh}）：Google/Provider表性别=${PERCEIVED_GENDER_LABEL_ZH[voice.perceivedGender]}；${voice.toneZh}；适合 ${voice.selectionHints.join("、")}`)
    .join("\n");
}
...
export function formatVoiceGenderSelectionRulesForPrompt(): string {
  ...
  return [
    "- Gemini TTS voice gender policy: Google/Provider voice tables classify every prebuilt Gemini TTS voice as either Female or Male; there is no neutral voice category.",
    `- When source role or voice metadata explicitly indicates female gender (女/女性/女子/姑娘/少女/女眷/mother/woman/girl/she/her), choose a female voice: ${femaleVoices}.`,
    `- When source role or voice metadata explicitly indicates male gender (男/男性/男子/少年/青年/先生/父亲/长者/man/boy/he/him), choose a male voice: ${maleVoices}.`,
    "- When gender is unknown, infer the intended speaker identity from role, age, scene, and transcript semantics, then choose a male or female voice that best matches both gender and style. Do not use or mention a neutral category.",
